...
namespace DAL1.DAO
{
    public class MaasDAO : PersonelDataContext
    {
        ...
        public static void MaasEkle(MAASST maas)
        {
            try
            {
                db.MAASSTs.InsertOnSubmit(maas);
                db.SubmitChanges();
            }
            catch (Exception ex)
            {
        ...
        public static void MaasGuncelle(MaasDetayDTO maas)
        {
            try
            {
                MAASST m = db.MAASSTs.First(x => x.ID == maas.MaasID);
        ...
        public static void MaasSil(int maasID)
        {
            try
            {
                MAASST maas = db.MAASSTs.First(x => x.ID == maasID);
                db.MAASSTs.DeleteOnSubmit(maas);
                db.SubmitChanges();  
            }
            catch (Exception ex)
            {
